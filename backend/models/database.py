...
class User(Base):
    ...
    id = Column(String, primary_key=True, index=True)  # Changed from user_id to id
    email = Column(String, unique=True, index=True, nullable=False)
    hashed_password = Column(String, nullable=False)  # Changed from password_hash to hashed_password
    is_active = Column(Boolean, default=True)  # Added missing column
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, onupdate=func.now())  # Changed from last_login to updated_at
    # Removed profile_data, learning_level, academic_background, robotics_experience as they don't exist in DB
