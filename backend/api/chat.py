    ...
    try:
        ...
        if not request.retrieved_context or len(request.retrieved_context) == 0:
            ...
            class QueryRequestInternal(BaseModel):
                ...
                provider: str = request.provider  # Use the same provider for querying
            ...
            class QueryResponse:
                def __init__(self, results, sources, provider):
                    self.results = results
                    self.sources = sources
                    self.provider = provider

            query_response = QueryResponse(results, sources, request.provider)
