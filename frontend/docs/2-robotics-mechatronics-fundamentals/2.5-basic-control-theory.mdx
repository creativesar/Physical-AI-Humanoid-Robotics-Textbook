...
*   **`D` (Derivative Term):**
    ...
    *   **Mathematical Representation:** $K_d \frac{de(t)}{dt}$
...
| $K_d$ | Minor change | Decrease | Decrease | No change | Improve |
...
3.  **Error Calculation:** The difference between the desired angle and the current angle ($e(t) = 90 - current\_angle$).
...
8.  What are the trade-offs between using a high versus low derivative gain ($K_d$)?